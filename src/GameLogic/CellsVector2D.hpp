...
class CellsVector2D {
  public:
    // Konstruktory:
    CellsVector2D();
    CellsVector2D( const std::size_t rowsCount, const std::size_t columnsCount, const std::size_t minesCount, std::size_t usedFlags = 0, bool mined = false );
    // Gettery:
  ...
  protected:
    // Składowe:
    std::size_t mRowsCount, mColumnsCount, mMinesCount, mUsedFlags;
